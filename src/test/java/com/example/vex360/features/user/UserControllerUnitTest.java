...
class UserControllerUnitTest {
        ...
        void updateCurrentUserProfileReturnsApiResponse() throws Exception {
                User user = User.builder().id(userId).email("user@example.com").role(Role.VISITOR)
                                .status(UserStatus.ACTIVE).build();
                UpdateProfileRequest request = new UpdateProfileRequest("User Name", "0912345678", "avatar.png");

                when(userService.updateCurrentUserProfile(any(String.class), any(UpdateProfileRequest.class)))
                                .thenReturn(response);
                authenticate(user);
