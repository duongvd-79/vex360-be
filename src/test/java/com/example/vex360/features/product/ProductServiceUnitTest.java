...
// import
// com.example.vex360.features.product.repositories.ProductCategoryRepository;
...
// @BeforeEach
// void setup() {
// service = new ProductService(companyService, categoryRepository,
// productRepository, cloudService, productMapper);
// user = User.builder().id(UUID.randomUUID()).build();
// company = Company.builder().id(UUID.randomUUID()).build();
// ProductContent content =
// ProductContent.builder().id(UUID.randomUUID()).publicId("content-id")
// .type(ProductContentType.VIDEO).build();
// product =
// Product.builder().id(UUID.randomUUID()).company(company).thumbnailPublicId("thumbnail-id")
