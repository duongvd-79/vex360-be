...
public class PackageTemplateController extends BaseController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PackageTemplateSelectionResponseDTO>>> getActivePackageTemplates() {
        List<PackageTemplateSelectionResponseDTO> templates = packageTemplateService.getActivePackageTemplates();
        return ok(templates);
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PackageTemplateSelectionResponseDTO>> getActivePackageTemplateById(
            @PathVariable UUID id) {
        PackageTemplateSelectionResponseDTO template = packageTemplateService.getActivePackageTemplateById(id);
        return ok(template);
    }
}
