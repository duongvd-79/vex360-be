...
public interface AnalyticsEventRepository extends JpaRepository<AnalyticsEvent, Long> {
    ...
    // ===== Thống kê cấp ban tổ chức (organizer) =====

    // Mỗi dòng = 1 triển lãm: [exhibitionId, số lượt vào triển lãm]
    // Gom 1 lần cho nhiều triển lãm -> tránh N+1 query ở màn tổng quan.
    @Query(value = """
    ...
    /*
     * Bảng xếp hạng gian hàng cho organizer: mỗi dòng = 1 gian hàng kèm tên triển
     * lãm, số khách, lượt xem và thời gian ở lại trung bình.
     *
     * Bắt đầu từ AnalyticsEvent để chỉ lấy gian hàng có phát sinh tương tác. Cách
     * này tránh LEFT JOIN ... ON với aggregate, vốn gây lỗi SQL trên một số bản
     * MySQL/Hibernate khi endpoint được gọi trên môi trường production.
     */
    @Query("""
            SELECT b.id,
                   b.name,
                   e.uuid,
                   e.name,
                   COUNT(DISTINCT CASE WHEN ae.eventType = :viewType THEN ae.user.id END),
                   COALESCE(SUM(CASE WHEN ae.eventType = :viewType THEN 1 ELSE 0 END), 0),
                   COALESCE(SUM(CASE WHEN ae.eventType = :chatType THEN 1 ELSE 0 END), 0),
                   AVG(CASE WHEN ae.eventType = :leaveType THEN ae.durationSeconds END)
            ...
            WHERE e.id IN :exhibitionIds
              ...
              AND ae.eventTime BETWEEN :start AND :end
            GROUP BY b.id, b.name, e.uuid, e.name
            """)
    List<Object[]> aggregateBoothRankingForOrganizer(
            @Param("exhibitionIds") List<Integer> exhibitionIds,
            @Param("viewType") AnalyticsEventType viewType,
            @Param("chatType") AnalyticsEventType chatType,
            @Param("leaveType") AnalyticsEventType leaveType,
            @Param("start") Instant start,
            @Param("end") Instant end);
    ...
    /** Mỗi dòng = một giờ: số khách khác nhau đã ENTER_EXHIBITION trong giờ đó. */
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<Object[]> aggregateExhibitionVisitorsByHour(
            @Param("exhibitionId") Integer exhibitionId,
            @Param("start") Instant start,
            @Param("end") Instant end);
