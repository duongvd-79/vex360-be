...
public class PayOSWebhookServiceImpl implements PayOSWebhookService {
    ...
    private void validateWebhookData(WebhookData data) {
        if (data == null || data.getOrderCode() == null || data.getAmount() == null
                || data.getDescription() == null || data.getAccountNumber() == null
                || data.getTransactionDateTime() == null || data.getCurrency() == null
                || data.getPaymentLinkId() == null || data.getCode() == null
                || data.getDesc() == null || data.getReference() == null) {
            log.error("Verified PayOS webhook is missing required fields");
            throw new AppException(ErrorCode.UNCATCHED_EXCEPTION);
        }
    }
