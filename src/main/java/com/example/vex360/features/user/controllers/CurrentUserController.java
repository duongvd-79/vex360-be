...
public class CurrentUserController extends BaseController {
        ...
        @GetMapping("/me")
        @Operation(summary = "Lay thong tin ca nhan", description = "Tra ve thong tin ho so cua user dang dang nhap.")
        ...
        @PatchMapping("/me")
        @Operation(summary = "Cap nhat ho so ca nhan", description = "Cap nhat fullName, phoneNumber va avatarUrl cua user dang dang nhap.")
        public ResponseEntity<ApiResponse<UserResponseDTO>> updateCurrentUserProfile(
                        @AuthenticationPrincipal UserDetails userDetails,
                        @Valid @RequestBody UpdateProfileRequest request) {
                UserResponseDTO user = userService.updateCurrentUserProfile(userDetails.getUsername(), request);
                return ok(user);
        }

        @PatchMapping("/me/password")
        @Operation(summary = "Doi mat khau ca nhan", description = "User dang dang nhap doi mat khau bang mat khau cu va mat khau moi.")
        public ResponseEntity<ApiResponse<Void>> changeCurrentUserPassword(
                        @AuthenticationPrincipal UserDetails userDetails,
                        @Valid @RequestBody ChangePasswordRequest request) {
                userService.changeCurrentUserPassword(userDetails.getUsername(), request);
                return ok(null);
        }
}
